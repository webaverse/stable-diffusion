        ...
        if self.path.startswith("/mod_mass"):
            ...
            def image_done(image, seed, upscaled=False):
                ...
                if opt.variation_amount > 0:
                    ...
                    iter_opt.seed = seed
                normalized_prompt = PromptFormatter(model, iter_opt).normalize_prompt()
            ...
            try:
                if opt.init_img is None:
                    # Run txt2img
                    model.prompt2image(**vars(opt), step_callback=image_progress, image_callback=image_done)
                else:
                    ...
                    try:
                        # Run img2img
                        model.prompt2image(**vars(opt1), step_callback=image_progress, image_callback=image_done)
                    finally:
